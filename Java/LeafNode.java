...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        //do we need to get a buffer?   RAM or disk?  pull out from disk?
        //how to add rid  or rid structure?
        //how to create new page for the new nodes

        int currSize = keys.size();
        int maxSize = 2 * metadata.getOrder();

        if (currSize <= maxSize) { //not overflowing
            this.sync();
        ...
        int splitSize = splitIndex + 1;
        for(int i = 0; i < splitSize; i++) {
            newKeys.add(this.keys.remove(splitIndex));
            newRids.add(this.rids.remove(splitIndex));
        }

        Optional<Long> oldRightSibling = this.rightSibling;
        LeafNode splitNode = new LeafNode(metadata, bufferManager, newKeys,
                newRids, oldRightSibling, treeContext);

        this.rightSibling = splitNode.rightSibling;
        this.sync();
        return Optional.of(new Pair<DataBox, Long>(splitNode.keys.get(0),
                                                   splitNode.page.getPageNum()));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int maxLoad = (int) Math.ceil(this.metadata.getOrder() * 2 * fillFactor);
        while(keys.size() < maxLoad && data.hasNext()) {
            Pair<DataBox, RecordId> currPair = data.next();
            this.keys.add(currPair.getFirst());
            this.rids.add(currPair.getSecond());
            this.sync();
        ...
        //split leaf node when overflow
        Pair<DataBox, RecordId> currPair = data.next();
        List<DataBox> nextKeys = new ArrayList<>();
        List<RecordId> nextRids = new ArrayList<>();
        nextKeys.add(currPair.getFirst());
        nextRids.add(currPair.getSecond());
        Optional<Long> nextRightSibling = Optional.empty();
        LeafNode nextNode = new LeafNode(metadata, bufferManager, nextKeys,
                nextRids, nextRightSibling, treeContext);

        //update current rightSib
        this.rightSibling = Optional.of(nextNode.getPage().getPageNum());

        this.sync();
        return Optional.of(new Pair<DataBox, Long>(nextNode.keys.get(0),
                nextNode.page.getPageNum()));
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // brand new one.
        Page currentPage = bufferManager.fetchPage(treeContext, pageNum);
        Buffer buf = currentPage.getBuffer();
        byte nodeType = buf.get();
        assert(nodeType == (byte) 1);

        Long getRightSib = buf.getLong();
        Optional<Long> currRightSibling;
        if (getRightSib == -1L) {
            currRightSibling = Optional.empty();
        } else {
            currRightSibling = Optional.of(getRightSib);
        }



        List<DataBox> keys = new ArrayList<>();
        List<RecordId> currRids = new ArrayList<>();


        int n = buf.getInt();
        for (int i = 0; i < n; ++i) {
            keys.add(DataBox.fromBytes(buf, metadata.getKeySchema()));
            currRids.add(RecordId.fromBytes(buf));
        }


        return new LeafNode(metadata, bufferManager, currentPage, keys,
                currRids, currRightSibling, treeContext);
    }
