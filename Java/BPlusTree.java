...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode currNode = root.getLeftmostLeaf();
        return new BPlusTreeIterator(currNode, 0);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LeafNode currNode = root.get(key);
        int index = InnerNode.numLessThan(key, currNode.getKeys());
        return new BPlusTreeIterator(currNode, index);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> updatedNode = root.put(key, rid);

        if (updatedNode.equals(Optional.empty())) {
            return;
        }

        List<DataBox> newRoot = new ArrayList<>();
        newRoot.add(updatedNode.get().getFirst());

        List<Long> newChildren = new ArrayList<>();
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(updatedNode.get().getSecond());
        this.updateRoot(new InnerNode(this.metadata,
                                      bufferManager,
                                      newRoot,
                                      newChildren,
                                      lockContext));
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        //check return: create an inner node
        if (this.scanAll().hasNext()) {
            throw new BPlusTreeException("Tree is not empty to bulk load!");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> updatedNode = root.bulkLoad(data, fillFactor);

            if (updatedNode.isPresent()) {
                List<DataBox> newRoot = new ArrayList<>();
                newRoot.add(updatedNode.get().getFirst());
                List<Long> newChildren = new ArrayList<>();
                newChildren.add(root.getPage().getPageNum());
                newChildren.add(updatedNode.get().getSecond());
                InnerNode newRootNode = new InnerNode(this.metadata,
                        this.bufferManager,
                        newRoot,
                        newChildren,
                        this.lockContext);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        // know where am I, which leaf node
        //class variable: leafnode, iterator
        //has next: if current iterator has some data/keys, still have sib
        //no more right sib
        //grab next staff
        // has next: do I have next staff to read,
        // rib, a new iterator. update the leaf and iterator
        private LeafNode currLeafNode;
        int currIndex = 0;
        int maxSize;

        private BPlusTreeIterator(LeafNode LeafNode, int index) {
            this.currLeafNode = LeafNode;
            this.currIndex = index;
            this.maxSize = this.currLeafNode.getKeys().size();
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            // return bufPos < RecordId.getSizeInBytes();leaf node has next key, check the size of RecordID list

            if (this.currIndex < this.maxSize){
                return true;
            }

            if (this.currLeafNode.getRightSibling().isPresent()) {
                LeafNode nextLeafNode = this.currLeafNode.getRightSibling().get();
                this.currIndex = 0;
                maxSize = nextLeafNode.getKeys().size();
                return true;
            }

            return false;

//           while(!currIte.hasNext() && currLeafNode.getRightSibling().isPresent()){
//               currLeafNode = currLeafNode.getRightSibling().get();
//               currIte = currLeafNode.scanAll();
//
//           }
        ...
        @Override
        public RecordId next() { // get next key
            // TODO(proj2): implement
            RecordId currRecord = this.currLeafNode.getRids().get(currIndex);
            currIndex += 1;
            return currRecord;
        }
    }
