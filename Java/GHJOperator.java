...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        // Building stage
        for (Record smallRecord: buildRecords) {
            DataBox smallJoinValue = smallRecord.getValue(buildColumnIndex);
            if (!hashTable.containsKey(smallJoinValue)) {
                hashTable.put(smallJoinValue, new ArrayList<>());
            }
            hashTable.get(smallJoinValue).add(smallRecord);
        }

        // Probing stage
        for (Record largeRecord: probeRecords) {
            DataBox largeJoinValue = largeRecord.getValue(probeColumnIndex);
            if (!hashTable.containsKey(largeJoinValue)) continue;
            // We have to join the right record with each left record with
            // a matching key
            for (Record lRecord : hashTable.get(largeJoinValue)) {
                Record joinedRecord = lRecord.concat(largeRecord);
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() > maxLen && rightPartitions[i].getNumPages() > maxLen) {
