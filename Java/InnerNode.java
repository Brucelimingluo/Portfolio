...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        //recursive
        //check if notetype == 1
        // yes: leafnode; no: continue
        //check children list: go left or right?
        //call left or right
        int index = this.numLessThanEqual(key, keys);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        this.sync();
        int childIndex = this.numLessThanEqual(key, keys);
        Optional<Pair<DataBox, Long>> newInnerNode = this.getChild(childIndex).put(key, rid);

        if (newInnerNode.equals(Optional.empty())) {//check if anything is returned
            return Optional.empty();
        }

        DataBox updatedKey = newInnerNode.get().getFirst();

        int newKeyIndex = this.numLessThanEqual(updatedKey, keys);
        this.keys.add(newKeyIndex, updatedKey);
        this.children.add(newKeyIndex + 1, newInnerNode.get().getSecond());
        ...
        for(int i = 0; i < splitIndex; i++) {
            newKeys.add(this.keys.remove(splitIndex + 1));
            splitChildren.add(this.children.remove(splitIndex + 1));
        }
        splitChildren.add(this.children.remove(splitIndex + 1)); //children have one more entry than keys
        DataBox val = this.keys.remove(splitIndex);

        InnerNode splitInnerNode = new InnerNode(metadata, bufferManager, newKeys, splitChildren, treeContext);

        this.sync();
        return Optional.of(new Pair<DataBox, Long>(val, splitInnerNode.page.getPageNum()));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> newInnerNode = this.getChild(this.children.size()-1).bulkLoad(data, fillFactor);
            ...
            DataBox updatedKey = newInnerNode.get().getFirst();
            this.keys.add(updatedKey);
            this.children.add(newInnerNode.get().getSecond());

            int currSize = keys.size();//check the current inner node size
            int maxSize = 2 * metadata.getOrder();
            if (currSize <= maxSize) { //not overflowing
                this.sync();
                return Optional.empty();
