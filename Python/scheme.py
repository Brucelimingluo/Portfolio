...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(operators)
        return scheme_apply(operators, rest.map(lambda a: scheme_eval(a, env)), env)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        selfname = self
        while selfname is not None:
            if symbol in selfname.bindings:
                return selfname.bindings[symbol]
            selfname = selfname.parent
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        result = expressions.first.first
        env.define(result, do_lambda_form(Pair(expressions.first.rest, expressions.rest), env))
        return result
    else:
